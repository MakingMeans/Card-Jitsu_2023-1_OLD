...
public class Controller implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "btnCrear": {
			userCreate.setYesTxt("SignUp", 35f, 192, 114);
			userCreate.setNoTxt("Volver", 35f, 36, 114);
		...
		case "btnConfirmCrear": {
			...
			if (error) {
				JOptionPane.showMessageDialog(userCreate, "Usuario repetido o vacío.", "Error",
						JOptionPane.ERROR_MESSAGE);
			} else if (!error) {
				JOptionPane.showMessageDialog(userCreate, "Usuario creado con éxito.", "Validación",
		...
		case "btnIngresar": {
			userLogin.setYesTxt("LogIn", 35f, 205, 114);
			userLogin.setNoTxt("Volver", 35f, 35, 114);
			inicial.setVisible(false);
		...
		case "btnConfirmLogin": {
			...
			if (error) {
				JOptionPane.showMessageDialog(userLogin, "No existe tal usuario.", "Error", JOptionPane.ERROR_MESSAGE);
			} else if (!error) {
				...
				if (name.equals("rickroll69")) {
					JOptionPane.showMessageDialog(userCreate, "Corre.", "Desesperación", JOptionPane.WARNING_MESSAGE);
				...
				Runnable timeCallback = () -> {
					Runnable animeCallback = () -> {
						...
						if (gameLogic.getPerdedor()) {
							...
							music.shutUp();
							if (funcionesPrimarias.currentUser().getName().equals("rickroll69"))
								music.soundEffect("Meme");
							else
								music.soundEffect("TimeIsUp");
					...
					if (gameLogic.getWinRound()) {
						...
						music.soundEffect("Meme");
						gameplay.penguinAnime(ogCard, "Empate", animeCallback);
					}
				};
		...
		case "btnCustomMaz": {
			...
			try {
				...
				JOptionPane.showMessageDialog(mazoView, "El número de la carta debe ser númerico (entre 2 y 15).",
						"Error", JOptionPane.ERROR_MESSAGE);
				break;
			}
			if (preId.isEmpty()) {
				JOptionPane.showMessageDialog(mazoView, "La ID de la carta esta vacía.", "Error",
						JOptionPane.ERROR_MESSAGE);
				break;
			}
			if (preNum.isEmpty()) {
				JOptionPane.showMessageDialog(mazoView, "No se especificó el número de la carta.", "Error",
						JOptionPane.ERROR_MESSAGE);
				break;
			}
			...
			if (mazoView.getElement() == null) {
				JOptionPane.showMessageDialog(mazoView, "No ha seleccionado algún elemento.", "Error",
						JOptionPane.ERROR_MESSAGE);
				break;
			}
			if (mazoView.getColor() == null) {
				JOptionPane.showMessageDialog(mazoView, "No ha seleccionado algún color.", "Error",
						JOptionPane.ERROR_MESSAGE);
				break;
			}
			byte error2 = funcionesSecundarias.asignarMazoCustom(name, mazoView.getId(), mazoView.getNum(),
					mazoView.getElement(), mazoView.getColor());
			;
			if (error2 != 0) {
				JOptionPane.showMessageDialog(mazoView, "No se puede crear la Carta #" + (mazoView.getId() + 1)
						+ " porque " + mazoView.getNum() + " de " + mazoView.getElement() + " " + mazoView.getColor()
						+ " ya esta en el mazo, no pueden haber cartas repetidas. Revisar Carta #" + error2 + ".",
						"Error", JOptionPane.ERROR_MESSAGE);
			} else {
				mazoView.setCurrentMazo(funcionesPrimarias.currentUser().getMazo());
				;
				mazoView.setBigCard(String.valueOf(mazoView.getNum()), mazoView.getElement(), mazoView.getColor());
				JOptionPane.showMessageDialog(userCreate,
						"Se ha remplazado la Carta #" + (mazoView.getId() + 1) + " por una Carta " + mazoView.getNum()
								+ " de " + mazoView.getElement() + " " + mazoView.getColor() + ".",
