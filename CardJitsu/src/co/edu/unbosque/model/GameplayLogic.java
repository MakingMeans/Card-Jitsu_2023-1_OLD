...
public class GameplayLogic {
	...
	public boolean ganador(boolean hayGanador) {
		// mecanismo de alamacenamiento para victorias de elementos iguales con colores
		// diferentes
		for (int i = 0; i < winsPlayer.size(); i++) {
			if (winsPlayer.get(i).getElemento().equals("Fuego")
					&& !playerFuego.contains(winsPlayer.get(i).getColor())) {
				playerFuego.add(winsPlayer.get(i).getColor());
			} else if (winsPlayer.get(i).getElemento().equals("Agua")
					&& !playerAgua.contains(winsPlayer.get(i).getColor())) {
				playerAgua.add(winsPlayer.get(i).getColor());
			} else if (winsPlayer.get(i).getElemento().equals("Nieve")
					&& !playerNieve.contains(winsPlayer.get(i).getColor())) {
		...
		for (int i = 0; i < winsRival.size(); i++) {
			...
			} else if (winsRival.get(i).getElemento().equals("Agua")
					&& !rivalAgua.contains(winsRival.get(i).getColor())) {
				rivalAgua.add(winsRival.get(i).getColor());
			} else if (winsRival.get(i).getElemento().equals("Nieve")
					&& !rivalNieve.contains(winsRival.get(i).getColor())) {
				rivalNieve.add(winsRival.get(i).getColor());
			}
		}
		// mecanismo de combinaciones sin repeticion entre las cartas y sus colores,
		// para mecanismo de vicotrias de elementos diferentes con colores diferentes
		for (int i = 0; i < playerFuego.size(); i++) {
			for (int j = 0; j < playerAgua.size(); j++) {
				for (int k = 0; k < playerNieve.size(); k++) {
					if (!playerFuego.get(i).equals(playerAgua.get(j)) && !playerFuego.get(i).equals(playerNieve.get(k))
							&& !playerAgua.get(j).equals(playerNieve.get(k))) {
		...
		for (int i = 0; i < rivalFuego.size(); i++) {
			for (int j = 0; j < rivalAgua.size(); j++) {
				for (int k = 0; k < rivalNieve.size(); k++) {
					if (!rivalFuego.get(i).equals(rivalAgua.get(j)) && !rivalFuego.get(i).equals(rivalNieve.get(k))
							&& !rivalAgua.get(j).equals(rivalNieve.get(k))) {
		...
		if (playerFuego.size() >= 3 || playerAgua.size() >= 3 || playerNieve.size() >= 3
				|| playerElementoDiferente.size() >= 3) {
		...
		} else if (rivalFuego.size() >= 3 || rivalAgua.size() >= 3 || rivalNieve.size() >= 3
				|| rivalElementoDiferente.size() >= 3) {
